...
func buildRouter(cfg *config.Config, ca *pki.CA, nodeSvc *node.Service, cfgStore *configstore.ConfigStore, sessions *session.SessionManager, agentSrv *transport.Server, semantic *configstore.SemanticChecker, drift *configstore.DriftDetector, tmplSvc *configstore.TemplateService, deploySvc *deploy.Service, hub *Hub, certSvc *cert.Service, lvsSvc *lvs.Service, logStore logstore.Storage, secSvc security.EventStore, blockSvc *security.BlockService) *gin.Engine {
	...
	seh := handler.NewSecurityHandler(secSvc, blockSvc)
