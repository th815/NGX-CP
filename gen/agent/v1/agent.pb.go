...
const (
	...
	HeartbeatRequest_LOG_BATCH       HeartbeatRequest_Type = 12 // T063：日志批次上报（Agent tail → 控制面入库）
)
...
type HeartbeatRequest struct {
	...
	LogBatch          []byte                     `protobuf:"bytes,14,opt,name=log_batch,json=logBatch,proto3" json:"log_batch,omitempty"`                            // T063：日志批次（Agent tail → 控制面入库），JSON 编码的 LogBatchPayload
	unknownFields     protoimpl.UnknownFields
	sizeCache         protoimpl.SizeCache
}
...
// GetLogBatch 返回 T063 日志批次字节（JSON 编码的 LogBatchPayload）。
func (x *HeartbeatRequest) GetLogBatch() []byte {
	if x != nil {
		return x.LogBatch
	}
	return nil
