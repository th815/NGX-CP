...
> **范围**：192.168.5.6 / .7（LVS Director，主备，VIP `192.168.5.5`）+ 192.168.5.8 / .9（Nginx RealServer）
> **日期**：2026-09-04
...
| PASS | 26 |
| FLAG | 3 |
| 拓扑 | 2 Director + 2 RS，LVS-DR，wrr，persistent 60，VIP `192.168.5.5` |
...
**证据**（`keepalived_master.conf`）：配置中**无 `unicast_peer { … }` 块**，`vrrp_instance VI_1` 直接依赖默认组播；仅用 `auth_type PASS / auth_pass 1111`。
...
**证据**（`ipvsadm -Ln`）：VIP `192.168.5.5` 下存在**三条独立 VS 条目**：

```text
TCP  192.168.5.5:80    wrr persistent 60   → .8:80 / .9:80
TCP  192.168.5.5:443   wrr persistent 60   → .8:443 / .9:443     （HTTPS）
UDP  192.168.5.5:443   wrr persistent 60   → .8:443 / .9:443     （QUIC/HTTPS3）
```
...
- ✅ RS 上 VIP `192.168.5.5/32` 落 `lo`；`arp_ignore=1 / arp_announce=2` 正确（DR 硬约束）
