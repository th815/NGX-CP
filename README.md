...
⬜ **M6–M9**：日志与安全 / 监控 / 构建升级 / 备份运维（增值模块，可边用边做）。T063–T070 待做：标准日志格式下发(T060)、检索/全链路/TraceID/聚合 API(T063/T064/T065)、攻击检测规则引擎(T066)、告警中心(T067)、封禁变更单(复用发布流水线)(T068)、分级处置(T069)、日志/安全 UI(T070)。
