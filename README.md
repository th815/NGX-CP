...
⬜ **M6–M9**：日志与安全 / 监控 / 构建升级 / 备份运维（增值模块，可边用边做）。T060/T066–T070 待做：标准日志格式下发 **T060**（让节点产出结构化 JSON 日志，使 TraceID 全链路、富字段聚合在真机也生效——当前 agent 已能读存量文本日志，但 rid/upstream 等富字段与跨节点追踪依赖 T060 的 `$request_id`/`$upstream_*`）、攻击检测规则引擎(T066)、告警中心(T067)、封禁变更单(复用发布流水线)(T068)、分级处置(T069)、日志/安全 UI(T070)。
